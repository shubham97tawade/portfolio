...
export default function BlogPreview({ blog }: BlogPreviewProps){
    ...
    const navigateToBlog = () => {
        router.push(`/blogs/blog/${blog.blog_id}`)
    }
    return(
        <section className="grid grid-rows-2 md:grid-cols-2 w-5/6 md:w-2/3 bg-white rounded overflow-hidden h-[32rem] md:h-80">
            ...
            <article className="grid-row p-6 md:p-8">
                ...
                <button className="mt-4 items-center px-4 py-3 justify-center bg-black text-white text-base rounded-lg transition duration-300 shadow-md hover:shadow-sm focus:outline-none" onClick={navigateToBlog}>Read article</button>
            </article>
        </section>
    );
}